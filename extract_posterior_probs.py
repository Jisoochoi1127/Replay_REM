...
for condition, mouse in tqdm(list(itertools.product(condition_list,
    ...
    if not os.path.exists(os.path.join(params['path_to_output'],"posterior_probs", f'posterior_probs_{condition}_{mouse}.h5')):
        ...
        REMpre_posterior_probs, _ = bayesian_decode(tuning_curves[selected_neurons], occupancy, marginal_likelihood[selected_neurons], data_REMpre['binaryData'][:,selected_neurons])

        # Extract posterior probs on REMpost and save results
        REMpost_posterior_probs, _ = bayesian_decode(tuning_curves[selected_neurons], occupancy, marginal_likelihood[selected_neurons], data_REMpost['binaryData'][:,selected_neurons])
