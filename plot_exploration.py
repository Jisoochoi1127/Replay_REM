...
for condition, mouse in tqdm(
    ...
    try:
        plt.subplot(len(condition_list), len(mouse_list), ct)

        data = load_data(mouse=mouse, condition=condition, state="wake", params=params)
        plt.plot(data["position"][:, 0], data["position"][:, 1], "C4", linewidth=0.2)
        plt.plot([0, 100], [0, 0], "k", linewidth=0.6)
        plt.plot([0, 100], [15, 15], "k", linewidth=0.6)
...
plt.savefig("../../output_REM/exploration.pdf")

# %% Plot maze exploration using params.speed_threshold
ct = 1
plt.figure(figsize=(len(mouse_list), len(condition_list)))
for condition, mouse in tqdm(
    list(itertools.product(condition_list, mouse_list)),
    ...
    try:
        ...
        data = load_data(mouse=mouse, condition=condition, state="wake", params=params)
        plt.hist(
            data["position"][data["velocity"] > params["speed_threshold"], 0], bins=40
        )
