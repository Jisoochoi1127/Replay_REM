...
from pycaan.functions.metrics import extract_firing_properties, extract_total_distance_travelled
...
for condition, mouse in tqdm(list(itertools.product(condition_list,
    ...
    if not os.path.exists(os.path.join(params['path_to_output'],f'tuning_{condition}_{mouse}.h5')) or params['overwrite_mode']=='always':
        try:
            # Load data
            ...
            info, p_value, occupancy_frames, active_frames_in_bin, tuning_curves, peak_loc, peak_val = extract_tuning(data['binaryData'],data['position'][:,0],data['running_ts'],bin_vec)
