...
for file_name in tqdm(resultsList):
    if file_name.startswith("tuning_") and file_name.endswith(
        ".h5"
        ...
        sorted_activity = np.argsort(
            marginal_likelihood * -1
        )  # Find the most active neurons
        r_sorted_activity = np.argsort(
            marginal_likelihood
        ...
        try:
            ...
            print(
                "There might not be enough selected neurons. Try reducing the number in params"
            )
