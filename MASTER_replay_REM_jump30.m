function MASTER_replay_REM(overwrite_flag)
%% MASTER_REM_replay: Runs all elements of the REM replay project. 1) data loading, 2) Tuning curve extraction, 3) decoding, 4) replay detection, 5) replay selection, as well as figure generation.


%     Inputs:
%        - overwrite_flag: [string]     determines if the script will
%        overwrite previously generated intermediate files.
%               Options:
%                 -'Hard' : defualt which will overwrite everything
%                 -'Place' : will overwrite everything from loading data
%                 onwards
%                 -'Decoding' : will overwrite everything after  computing
%                 the place information.
%                 -'Replay' : overwrite  everything after decoding
%                 -'Selection': overwrite from replay selection onwards.
%                 -'Figures': defualt which will overwrite the figures only
%
...
fprintf('<strong>%s</strong>: Overwrite_flag set to <strong>%s</strong>\n', mfilename, overwrite_flag);
...
if strcmpi(overwrite_flag, 'hard')
...
if contains(overwrite_flag, {'Hard', 'Place'})
...
%% Decoding
if contains(overwrite_flag, {'Hard','Place', 'Decoding'})
    ...
    parfor iF = 1:length(fnames)
        ...
        Decoding_batch(PARAMS, fnames(iF).name, fnames_PC(iF).name, decoding_dir)
%         warning off
%         load(fnames(iF).name)
...
%% Replay
if contains(overwrite_flag, {'Hard','Place', 'Decoding', 'Replay'})
...
%% Replay selection
if contains(overwrite_flag, {'Hard','Place', 'Decoding', 'Replay', 'Selection'})
...
%% Figures
if contains(overwrite_flag, {'Hard','Place', 'Decoding', 'Replay', 'Selection', 'Figures'})
